...
if isfield(data.epocs,'freq')
    freqAll = data.epocs.freq.data(1:length(data.epocs.freq.data)/length(onsetTimeAll):end); % Hz
elseif isfield(data.epocs,'vair')
    freqAll = data.epocs.vair.data(1:length(data.epocs.vair.data)/length(onsetTimeAll):end); % Hz
end
if isfield(data.epocs,'attv')
    attAll = data.epocs.attv.data(1:length(data.epocs.attv.data)/length(onsetTimeAll):end); % absolute attenuation, dB
elseif isfield(data.epocs,'var2')
    attAll = data.epocs.var2.data(1:length(data.epocs.var2.data)/length(onsetTimeAll):end); % absolute attenuation, dB
end
