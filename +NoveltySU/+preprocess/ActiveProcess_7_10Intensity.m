function trialAll = ActiveProcess_7_10Intensity(epocs, choiceWin)
    narginchk(1, 2);
...
%     end
    ...
    for tIndex = 1:length(trialOnsetIndex) - 1
        ...
        trialAll(tIndex, 1).intSeq = intAll(soundOnsetIndex);
        ...
        if ~isempty(find(errorPushTimeAll >= trialAll(tIndex, 1).soundOnsetSeq(end) & errorPushTimeAll < trialOnsetTimeAll(tIndex + 1), 1))
            ...
            if trialAll(tIndex, 1).intSeq(end) ~= trialAll(tIndex, 1).intSeq(1)
                trialAll(tIndex, 1).devInt = trialAll(tIndex, 1).intSeq(end);
                trialAll(tIndex, 1).stdNum = length(trialAll(tIndex, 1).soundOnsetSeq) - 1;
            else
                trialAll(tIndex, 1).devInt = 0;
            ...
            trialAll(tIndex, 1).devInt = trialAll(tIndex, 1).intSeq(end);
            trialAll(tIndex, 1).stdNum = length(trialAll(tIndex, 1).intSeq) - 1;
