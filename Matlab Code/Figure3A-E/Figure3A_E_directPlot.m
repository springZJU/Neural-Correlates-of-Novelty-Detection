...
devStr = cellfun(@(x) num2str(roundn(x, -2)), num2cell(devType(2:4)), "UniformOutput", false);
for dIndex = 1 : 3
    mAxe(dIndex) = subplot(3, 3, dIndex);
    countDev = selectRes.spkDev.all(dIndex + 1).frRaw_Late / 10;
    devBarVal = histcounts(countDev, binEdges);
    ...
    xlabel("spike counts");
    ylabel("trial counts");
    title(['dev/std = ', char(devStr(dIndex))])
end
